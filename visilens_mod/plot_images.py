## Use of the "converged" model in the figures.
import numpy as np
import copy
import warnings
...
def uvimageslow(visdata,imsize=256,pixsize=0.5,taper=0.):
      ...
      print("Shape of thisvis.u:", thisvis.u.shape)
      print("Shape of first half:", thisvis.u[:thisvis.u.size//2].shape)
      print("Shape of second half:", thisvis.u[thisvis.u.size//2:].shape)
           
     #print(thisvis.u)

      with open('visibilities.dat', 'w') as f:
          for val in thisvis.u:
              f.write(f"{val:.3e}\n")


      # Check to see if we have the conjugate visibilities, and image.
      n_half = thisvis.u.size // 2
      if np.all(thisvis.u[:n_half] == -thisvis.u[-n_half:]):
...
def plot_images(data,mcmcresult,returnimages=False,plotcombined=False,plotall=False,
      ...
      lens = mcmcresult['last_50_steps']['lens']
      source=mcmcresult['last_50_steps']['source']
      scaleamp = mcmcresult['last_50_steps']['scaleamp'] if 'scaleamp' in mcmcresult['last_50_steps'].keys() else np.ones(len(datasets))
      shiftphase=mcmcresult['last_50_steps']['shiftphase'] if 'shiftphase' in mcmcresult['last_50_steps'].keys() else np.zeros((len(datasets),2))
