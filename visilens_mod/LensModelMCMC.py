### Modified code to calculate the "best-fit" of the last X steps of the chains.
import numpy as np
import scipy.sparse
import os
import sys
...
def LensModelMCMC(data,lens,source,
     ...
     # for l in blobs:
        ...
        #          mus_list.append(mu)
       #       else:
      #            mus_list.append(np.array([np.nan]*len(source)))

     # mus = np.asarray(mus_list, dtype=float)
...
################################################################################
      ### best-fit de los últimos steps
      print(type(mcmcresult['chains']))
      print(len(mcmcresult['chains']))
      print(np.array(mcmcresult['chains']).shape)

      # Keep track of best-fit params, derived from chains.
      c_2 = copy.deepcopy(mcmcresult['chains'][-100:])
      mcmcresult['last_50_steps'] = {}
      pbest = []
      # Calculate the best fit values as medians of each param
      lens,source = copy.deepcopy(mcmcresult['lens_p0']), copy.deepcopy(mcmcresult['source_p0'])
      for i,ilens in enumerate(lens):
            if ilens.__class__.__name__ == 'SIELens':
                  ...
                  for key in ['x','y','M','e','PA']:
                        if not vars(ilens)[key]['fixed']:
                              ilens.__dict__[key]['value'] = np.median(c_2[key+'L'+str(i)])
                              pbest.append(np.median(c_2[key+'L'+str(i)]))
            elif ilens.__class__.__name__ == 'ExternalShear':
                  for key in ['shear','shearangle']:
                        if not vars(ilens)[key]['fixed']:
                              ilens.__dict__[key]['value'] = np.median(c_2[key])
                              pbest.append(np.median(c_2[key]))
      
      mcmcresult['last_50_steps']['lens'] = lens
      ...
      for i,src in enumerate(source): # Source is a list of source objects
            if src.__class__.__name__ == 'GaussSource':
                  for key in ['xoff','yoff','flux','width']:
                        if not vars(src)[key]['fixed']:
                              src.__dict__[key]['value'] = np.median(c_2[key+'S'+str(i)])
                              pbest.append(np.median(c_2[key+'S'+str(i)]))
            elif src.__class__.__name__ == 'SersicSource':
                  for key in ['xoff','yoff','flux','majax','index','axisratio','PA']:
                        if not vars(src)[key]['fixed']:
                              src.__dict__[key]['value'] = np.median(c_2[key+'S'+str(i)])
      ...
      mcmcresult['last_50_steps']['source'] = source
      mcmcresult['last_50_steps']['magnification'] = np.median(mus[~bad],axis=0)
      ...
      if 'scaleamp' in mcmcresult.keys():
            for i,t in enumerate(mcmcresult['scaleamp']): # only matters if >1 datasets
                  if i==0: pass
                        ...
                        bfscaleamp[i] = np.median(c_2['ampscale_dset'+str(i)])
                        pbest.append(np.median(c_2['ampscale_dset'+str(i)]))
                  else: pass
      mcmcresult['last_50_steps']['scaleamp'] = bfscaleamp
      ...
      if 'shiftphase' in mcmcresult.keys():
            for i,t in enumerate(mcmcresult['shiftphase']):
                  if i==0: pass # only matters if >1 datasets
                        ...
                        bfshiftphase[i][0] = np.median(c_2['astromshift_x_dset'+str(i)])
                        bfshiftphase[i][1] = np.median(c_2['astromshift_y_dset'+str(i)])
                        pbest.append(np.median(c_2['astromshift_x_dset'+str(i)]))
                        pbest.append(np.median(c_2['astromshift_y_dset'+str(i)]))
                  else: pass # no shifting
      mcmcresult['last_50_steps']['shiftphase'] = bfshiftphase
      
      mcmcresult['last_50_steps']['lnlike'] = calc_vis_lnlike(pbest,data,mcmcresult['last_50_steps']['lens'],
            mcmcresult['last_50_steps']['source'],
            Dd,Ds,Dds,ug,xmap,ymap,xemit,yemit,indices,
            sourcedatamap,scaleamp,shiftphase,modelcal)[0]
      
      # Calculate the deviance information criterion, using the Spiegelhalter+02 definition (cf Gelman+04)
      mcmcresult['last_50_steps']['DIC'] = -4*np.mean(mcmcresult['lnlike']) + 2*mcmcresult['last_50_steps']['lnlike']
      ...
      ### best-fit de los últimos 50 steps probando de forma diferente
"""
      # Keep track of best-fit params, derived from chains.
      c_3 = copy.deepcopy(mcmcresult['chains'][;,-20:,:]) ## para este ejemplo como he puesto solo 40 pruebo con 20
      mcmcresult['last_50_steps_2'] = {}
      pbest = []
      # Calculate the best fit values as medians of each param
      lens,source = copy.deepcopy(mcmcresult['lens_p0']), copy.deepcopy(mcmcresult['source_p0'])
      for i,ilens in enumerate(lens):
            if ilens.__class__.__name__ == 'SIELens':
                  ...
                  for key in ['x','y','M','e','PA']:
                        if not vars(ilens)[key]['fixed']:
                              ilens.__dict__[key]['value'] = np.median(c_3[key+'L'+str(i)])
                              pbest.append(np.median(c_3[key+'L'+str(i)]))
            elif ilens.__class__.__name__ == 'ExternalShear':
                  for key in ['shear','shearangle']:
                        if not vars(ilens)[key]['fixed']:
                              ilens.__dict__[key]['value'] = np.median(c_3[key])
                              pbest.append(np.median(c_3[key]))
      
      mcmcresult['last_50_steps_2']['lens'] = lens
      ...
      for i,src in enumerate(source): # Source is a list of source objects
            if src.__class__.__name__ == 'GaussSource':
                  for key in ['xoff','yoff','flux','width']:
                        if not vars(src)[key]['fixed']:
                              src.__dict__[key]['value'] = np.median(c_3[key+'S'+str(i)])
                              pbest.append(np.median(c_3[key+'S'+str(i)]))
            elif src.__class__.__name__ == 'SersicSource':
                  for key in ['xoff','yoff','flux','majax','index','axisratio','PA']:
                        if not vars(src)[key]['fixed']:
                              src.__dict__[key]['value'] = np.median(c_3[key+'S'+str(i)])
      ...
      mcmcresult['last_50_steps_2']['source'] = source
      mcmcresult['last_50_steps_2']['magnification'] = np.median(mus[~bad],axis=0)
      ...
      if 'scaleamp' in mcmcresult.keys():
            for i,t in enumerate(mcmcresult['scaleamp']): # only matters if >1 datasets
                  if i==0: pass
                        ...
                        bfscaleamp[i] = np.median(c_3['ampscale_dset'+str(i)])
                        pbest.append(np.median(c_3['ampscale_dset'+str(i)]))
                  else: pass
      mcmcresult['last_50_steps_2']['scaleamp'] = bfscaleamp
      ...
      if 'shiftphase' in mcmcresult.keys():
            for i,t in enumerate(mcmcresult['shiftphase']):
                  if i==0: pass # only matters if >1 datasets
                        ...
                        bfshiftphase[i][0] = np.median(c_3['astromshift_x_dset'+str(i)])
                        bfshiftphase[i][1] = np.median(c_3['astromshift_y_dset'+str(i)])
                        pbest.append(np.median(c_3['astromshift_x_dset'+str(i)]))
                        pbest.append(np.median(c_3['astromshift_y_dset'+str(i)]))
                  else: pass # no shifting
      mcmcresult['last_50_steps_2']['shiftphase'] = bfshiftphase
      
      mcmcresult['last_50_steps_2']['lnlike'] = calc_vis_lnlike(pbest,data,mcmcresult['last_50_steps_2']['lens'],
            mcmcresult['last_50_steps_2']['source'],
            Dd,Ds,Dds,ug,xmap,ymap,xemit,yemit,indices,
            sourcedatamap,scaleamp,shiftphase,modelcal)[0]
      
      # Calculate the deviance information criterion, using the Spiegelhalter+02 definition (cf Gelman+04)
      mcmcresult['last_50_steps_2']['DIC'] = -4*np.mean(mcmcresult['lnlike']) + 2*mcmcresult['last_50_steps_2']['lnlike']
"""
###############################################3      
