## Using only the last 1000 steps of the MCMC chain, discarding everything before.
...
# Using only the last 1000 steps of the MCMC chain, discarding everything before.
                  x,y = copy.deepcopy(mcmcresult['chains'][xax][-1000:]), copy.deepcopy(mcmcresult['chains'][yax][-1000:])
                  ...
                  if row > col:
                        try: marginalize_2d(x,y,axarr[row,col],\
                              extent=[xmin,xmax,ymin,ymax],bins=int(max(np.floor(x.size/1000),50)))   #### 1000/50
                        ...
                        marginalize_1d(x,axarr[row,col],extent=[xmin,xmax],\
                              bins=int(max(np.floor(x.size/1000),50)))  ## 1000,50
      ...
      axarr[0,-1].text(-0.8,0.7-it,'DIC = {0:.0f}'.format(mcmcresult['last_50_steps']['DIC']),fontsize='xx-large',\
...
def marginalize_2d(x,y,axobj,*args,**kwargs):
      ...
      bins = int(kwargs.pop('bins',50)) #### por defecto está en 50, puedo reducir este número para hacer que los datos se dividan en menos bins y que salga más general el histograma
...
def marginalize_1d(x,axobj,*args,**kwargs):
      ...
      axobj.hist(x,bins,range=extent,histtype='stepfilled',color=fillcolor)
