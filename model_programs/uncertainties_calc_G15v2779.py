...
z_lens = 0.5950
# Lens distances
Dl = Planck18.angular_diameter_distance(z_lens)
Ds = Planck18.angular_diameter_distance(z_source)
Dls = (Planck18.angular_diameter_distance_z1z2(z_lens, z_source))

einstein_results = {}

# Detect lenses automatically
lens_ids = []
for name in param_names:
...
for lid in lens_ids:
	ml_name = f"ML{lid}"
	if ml_name not in results:
		continue
# Lens mass
	M_value = results[ml_name]["median"]
	M = M_value * u.solMass
	thetaE_rad = np.sqrt((4*G*M/c**2)*(Dls/(Dl*Ds)))
	thetaE_arcsec = thetaE_rad.to(u.arcsec,equivalencies=u.dimensionless_angles()).value

	sigma68_M = np.mean(results[ml_name]["err68"])
	sigma95_M = np.mean(results[ml_name]["err95"])
	sigma_std_M = results[ml_name]["std"]
# Error propagation 
	sigma68_theta = (0.5*thetaE_arcsec*sigma68_M / M_value)
	sigma95_theta = (0.5*thetaE_arcsec*sigma95_M / M_value)
	sigma_std_theta = (0.5*thetaE_arcsec*sigma_std_M / M_value)

	einstein_results[lid] = {
		"thetaE_arcsec": thetaE_arcsec,
		"sigma68_theta": sigma68_theta,
		"sigma95_theta": sigma95_theta,
		"sigma_std_theta": sigma_std_theta
	}
...
for lid, res in einstein_results.items():

	print(f"\n--- Lens L{lid} ---")
	print(
		f"theta_E,L{lid} = "
		f"{res['thetaE_arcsec']:.5f} arcsec"
	)

	print(
		f"68% uncertainty : "
		f"± {res['sigma68_theta']:.5f} arcsec"
	)

	print(
		f"95% uncertainty : "
		f"± {res['sigma95_theta']:.5f} arcsec"
	)
	print(
		f"Std uncertainty : "
		f"± {res['sigma_std_theta']:.5f} arcsec"
	)
